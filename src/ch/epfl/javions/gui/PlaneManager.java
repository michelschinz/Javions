...
public final class PlaneManager {
    ...
    private Polyline polyLineForPlaneTrajectory(IcaoAddress address, ObservablePlaneState planeState) {
        ...
        planeState.trajectory().addListener((ListChangeListener<GeoPos>) c ->
                l.getPoints().setAll(trajectory(mapViewParametersProperty.get().zoomLevel(), planeState.trajectory())));
        mapViewParametersProperty.addListener((p, o, n) ->
                l.getPoints().setAll(trajectory(n.zoomLevel(), planeState.trajectory())));

        l.layoutXProperty().bind(Bindings.createDoubleBinding(
                () -> -mapViewParametersProperty.get().topLeft().getX(),
                mapViewParametersProperty));
        l.layoutYProperty().bind(Bindings.createDoubleBinding(
